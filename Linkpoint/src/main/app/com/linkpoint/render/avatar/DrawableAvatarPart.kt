...
import com.linkpoint.slproto.avatar.SLAnimatedMeshData
import com.linkpoint.slproto.avatar.SLMeshData
import com.linkpoint.slproto.avatar.SLPolyMesh
...
class DrawableAvatarPart : ResourceConsumer {
    ...
    Unit setTexture(com.linkpoint.render.glres.textures.GLTextureCache glTextureCache, UUID textureUUID) {
        synchronized (this.updateLock) {
            ...
            if (textureUUID != null && com.linkpoint.utils.UUIDPool.ZeroUUID == textureUUID) {
                textureUUID = null
            }
            
            // If texture hasn't changed, return early
            if (this.textureUUID != null ? this.textureUUID == textureUUID : textureUUID == null) {
                return
            }
            ...
            // Update texture UUID and request texture
            this.textureUUID = textureUUID
            
            // Create texture parameters for this avatar part
            com.linkpoint.render.tex.DrawableTextureParams textureParams = 
                com.linkpoint.render.tex.DrawableTextureParams.create(
                    textureUUID, this.faceIndex, this.avatarUUID)
            
            // Request the texture from cache
            com.linkpoint.res.textures.TextureCache.getInstance()
