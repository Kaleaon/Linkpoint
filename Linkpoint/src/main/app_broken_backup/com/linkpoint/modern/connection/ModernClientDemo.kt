...
class ModernClientDemo {
    private val TAG: String = "ModernClientDemo"
    ...
    fun testConnectionOnly(context: Context): CompletableFuture<Boolean> {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                com.linkpoint.modern.connection.ModernConnectionManager connectionManager = 
                    com.linkpoint.modern.connection.ModernConnectionManager(context)
                
                Log.i(TAG, "Connection manager created, state: " + connectionManager.getState())
                
                // Test basic connectivity without full authentication
                com.linkpoint.modern.connection.ConnectionDiagnostics diagnostics = 
                    com.linkpoint.modern.connection.ConnectionDiagnostics(context)
