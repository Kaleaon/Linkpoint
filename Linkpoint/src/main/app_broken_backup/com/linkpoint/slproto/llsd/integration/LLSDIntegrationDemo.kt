...
class LLSDIntegrationDemo {
    ...
    fun runDemo()  {
        try {
            ...
            var simpleXML: String = "<?xml version='1.0' encoding='UTF-8'?>\n" +
                    "<llsd>\n" +
                    "  <map>\n" +
                    "    <key>name</key>\n" +
            ...
            if (parsed is LLSDMap) {
                ...
                if (map.keyExists("name")) {
                    var name: String = map.byKey("name").asString()
                    Log.i(TAG, "Name: " + name)
                }
                
                if (map.keyExists("version")) {
                    var version: Int = map.byKey("version").asInt()
                    Log.i(TAG, "Version: " + version)
                }
                
                if (map.keyExists("enabled")) {
                    var enabled: Boolean = map.byKey("enabled").asBoolean()
            ...
            var serialized: String = LLSDIntegrationBridge.serializeToXML(testString)
    ...
    fun testFallback()  {
        try {
            ...
            var malformedXML: String = "not valid xml"
