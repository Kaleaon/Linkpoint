...
class SLAvatarControl : SLModule() {
    ...
    private const val UUID animUUID_Run = UUID.fromString("05ddbff8-aaa9-92a1-2b74-8fe77a29b445")
    private const val UUID animUUID_Softland = UUID.fromString("f4f00d6e-b9fe-9292-f4cb-0ae06ea58d57")
    private const val UUID animUUID_Stand = UUID.fromString("2408fe9e-df1d-1d7d-f4ff-1384fa7b350f")
    private const val UUID animUUID_Standup = UUID.fromString("3da1d753-028a-5446-24f3-9c9b856d9422")
    private const val UUID animUUID_Walk = UUID.fromString("6ed24bd8-91aa-4b12-ccc7-c97c857ab4e0")
    ...
    private val AgentPosition agentPosition = AgentPosition()
    private val LLVector3 agentUpdateCameraCenter = LLVector3()
    ...
    private val CameraParams cameraParams = CameraParams()
    private val Object cammingLock = Object()
    /* access modifiers changed from: private */
    public volatile Boolean enableAgentUpdates = false
    ...
    /* access modifiers changed from: private */
    val ResultHandler<SubscriptionSingleKey, MyAvatarState> myAvatarStateResultHandler
    private volatile Boolean needClearAnims = true
    private volatile Int needFastUpdates = 10
    private val SLParcelInfo parcelInfo = this.agentCircuit.getGridConnection().parcelInfo
    private val Object turningLock = Object()
    private Float turningSpeed = 0.0f
    private Long turningStartTime = 0
    private val UserManager userManager = UserManager.getUserManager(this.agentCircuit.getAgentUUID())

    private class AgentUpdateTimerTask : TimerTask() {
        private val Int scheduledInterval
    ...
    /* access modifiers changed from: private */
    public Unit SendAgentUpdate(SLDrawDistance sLDrawDistance) {
    ...
    public Unit ApplyAvatarAnimation(SLObjectAvatarInfo sLObjectAvatarInfo, AvatarAnimation avatarAnimation) {
    ...
    /* access modifiers changed from: package-private */
    public Unit DisableFastUpdates() {
        Debug.Log("AgentUpdate: Disabling fast updates.")
        rescheduleAgentUpdate()
    }

    /* access modifiers changed from: package-private */
    public Unit EnableFastUpdates() {
        Debug.Log("AgentUpdate: Enabling fast updates.")
        rescheduleAgentUpdate()
    }

    public Unit ForceSitOnObject(UUID uuid) {
        if (uuid != null) {
    ...
    public synchronized Unit ForceStand() {
    ...
    public Unit ScriptAnswerYes(UUID uuid, UUID uuid2, Int i) {
    ...
    public Unit SitOnObject(UUID uuid) {
    ...
    public synchronized Unit Stand() {
        if (this.agentCircuit.getModules().rlvController.canStandUp()) {
            ForceStand()
        }
    }

    public Unit StartAgentMotion(Int i) {
        Boolean z = false
        synchronized (this) {
            if (!((i & 8) == 0 && (i & 16) == 0)) {
                if (!this.isFlying) {
    ...
    public synchronized Unit StopAgentMotion() {
    ...
    public Unit StopAvatarAnimations() {
    ...
    public Unit getVRCamera(HeadTransformCompat headTransformCompat, LLVector3 lLVector3, CameraParams cameraParams2) {
        this.agentPosition.getInterpolatedPosition(lLVector3)
        synchronized (this.cammingLock) {
    ...
    public Unit playAnimation(UUID uuid, Boolean z) {
        AgentAnimation agentAnimation = AgentAnimation()
        agentAnimation.AgentData_Field.AgentID = this.circuitInfo.agentID
        agentAnimation.AgentData_Field.SessionID = this.circuitInfo.sessionID
        AgentAnimation.AnimationList animationList = AgentAnimation.AnimationList()
    ...
    public Unit processCameraFling(Float f, Float f2) {
        synchronized (this.cammingLock) {
            this.cameraParams.fling(f, f2)
        }
    }

    public Unit processCameraRotate(Float f, Float f2) {
        synchronized (this.cammingLock) {
    ...
    public Unit processCameraZoom(Float f, Float f2, Float f3, Float f4, Float f5) {
        synchronized (this.cammingLock) {
            this.isCamming = true
    ...
    public Unit setAgentHeading(Float f) {
        synchronized (this.cammingLock) {
    ...
    public Unit setAgentPosition(LLVector3 lLVector3, LLVector3 lLVector32) {
        synchronized (this.cammingLock) {
    ...
    public Unit setCameraManualControl(Boolean z) {
        synchronized (this.cammingLock) {
    ...
    public Unit setEnableAgentUpdates(Boolean z) {
    ...
    public Unit startCameraManualControl(Float f, Float f2, Float f3, Float f4) {
        synchronized (this.cammingLock) {
            this.isCamming = true
    ...
    public Unit startTurning(Float f) {
        synchronized (this.turningLock) {
    ...
    public Unit stopCameraManualControl() {
        synchronized (this.cammingLock) {
            this.cameraParams.stopManualControl()
        }
    }

    public Unit stopCamming() {
        synchronized (this.cammingLock) {
            if (this.isCamming) {
                this.isCamming = false
                if (!this.isManualCamming) {
