...
class SLWorldMap : SLModule() {
    ...
    fun HandleFindAgent(findAgent: FindAgent) {
        if (this.teleportToAgentUUID != null && findAgent.AgentBlock_Field.Prey.equals(this.teleportToAgentUUID)) {
            ...
            if (findAgent.LocationBlock_Fields.size() > 0) {
                val d: Double = findAgent.LocationBlock_Fields.get(0).GlobalX
                val d2: Double = findAgent.LocationBlock_Fields.get(0).GlobalY
                if (!(d == 0.0d && d2 == 0.0d)) {
                    val floor: Int = (Int) Math.floor(d)
                    val floor2: Int = (Int) Math.floor(d2)
                    val i: Int = floor & 255
                    val i2: Int = floor2 & 255
                    val j: Long = (((Long) (floor & InputDeviceCompat.SOURCE_ANY)) << 32) | (((Long) (floor2 & InputDeviceCompat.SOURCE_ANY)) & 4294967295L)
    ...
    fun HandleMapBlockReply(mapBlockReply: MapBlockReply) {
        ...
        val it: Iterator<T> = mapBlockReply.Data_Fields.iterator()
        while (true) {
            z = z3
            if (!it.hasNext()) {
                break
            ...
            val stringFromVariableOEM: String = SLMessage.stringFromVariableOEM(data.Name)
            val j: Long = (((Long) (data.X * 256)) << 32) | (((Long) (data.Y * 256)) & 4294967295L)
    ...
    public fun TeleportToRegionByName(str: String, i: Int, i2: Int, i3: Int): Boolean {
        if (!this.agentCircuit.getModules().rlvController.canTeleportToLocation()) {
            return false
        }
        ...
        val mapNameRequest: MapNameRequest = MapNameRequest()
