...
class EmulatorManager {
    ...
    class EmulatorDefaults {
        ...
        const val String[] SUPPORTED_TAGS = {"google_apis", "google_apis_playstore", "default"}
        const val String[] POPULAR_DEVICES = {"pixel_2", "pixel_3", "pixel_4", "pixel_5", "Nexus_5X"}
    }
    ...
    private class EmulatorTask : AsyncTask()<Void, String, String> {
        private val String[] command
        private val EmulatorCallback callback
        ...
        override protected Unit onProgressUpdate(String... values) {
            if (callback != null && values.length > 0) {
                callback.onStatusUpdate(values[0])
            }
        }
        
        override protected Unit onPostExecute(String result) {
            if (callback != null) {
                if (hasError) {
