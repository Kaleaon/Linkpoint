...
class MinimapView : View() {
    ...
    private val ScaleGestureDetector.OnScaleGestureListener scaleGestureListener = ScaleGestureDetector.SimpleOnScaleGestureListener() {
         public fun onScale(scaleGestureDetector: ScaleGestureDetector): Boolean {
            val unused: Float = MinimapView.this.actualZoomFactor = Math.min(Math.max(MinimapView.this.actualZoomFactor * scaleGestureDetector.getScaleFactor(), 1.0f), 5.0f)
    ...
    public MinimapView(Context context, AttributeSet attributeSet, Int i) {
     ...
     private fun drawUserMark(immutableVector: ImmutableVector, canvas: Canvas, paint: Paint, rect: Rect, z: Boolean, f: Float, z2: Boolean) {
        val x: Float = immutableVector.getX()
        val y: Float = immutableVector.getY()
        val width: Float = ((Float) rect.left) + ((x / 256.0f) * ((Float) rect.width()))
        val width2: Float = ((Float) rect.top) + (((256.0f - y) / 256.0f) * ((Float) rect.width()))
        if (!z) {
        ...
        canvas.drawCircle(width, width2, 5.0f, paint)
        if (z && (!Float.isNaN(f))) {
            val cos: Float = (Float) (Math.cos((Double) f) * 20.0d)
            val sin: Float = (Float) (Math.sin((Double) f) * 20.0d)
            val cos2: Float = (Float) ((Math.cos((Double) f) * 15.0d) - (Math.sin((Double) f) * -5.0d))
            val cos3: Float = (Float) ((Math.cos((Double) f) * -5.0d) + (Math.sin((Double) f) * 15.0d))
            val cos4: Float = (Float) ((Math.cos((Double) f) * 15.0d) - (Math.sin((Double) f) * 5.0d))
            val cos5: Float = (Float) ((Math.cos((Double) f) * 5.0d) + (Math.sin((Double) f) * 15.0d))
     ...
     private fun handleTouch(f: Float, f2: Float) {
        ...
        if (this.userLocations != null && this.lastDrawRect != null) {
            val applyDimension: Float = TypedValue.applyDimension(1, USER_MARK_TOUCH_SLACK, getResources().getDisplayMetrics())
            val f3: Float = 0.0f
            val it: Iterator<T> = this.userLocations.userPositions.entrySet().iterator()
            while (true) {
                uuid = uuid2
                val f4: Float = f3
                if (!it.hasNext()) {
                ...
                val immutableVector: ImmutableVector = ((SLMinimap.UserLocation) entry.getValue()).location
                val abs: Float = Math.abs((((immutableVector.getX() / 256.0f) * ((Float) this.lastDrawRect.width())) + ((Float) this.lastDrawRect.left)) - f)
                val abs2: Float = Math.abs(((((256.0f - immutableVector.getY()) / 256.0f) * ((Float) this.lastDrawRect.width())) + ((Float) this.lastDrawRect.top)) - f2)
    ...
    override fun onDraw(canvas: Canvas) {
        if (this.minimapBitmap != null) {
            ...
            val round: Int = Math.round(((Float) Math.min(width, height)) * this.actualZoomFactor)
            val i: Int = width / 2
            val i2: Int = height / 2
            if (round <= width) {
            ...
            val i3: Int = (i - (round / 2)) + ((Int) this.mapOffsetX)
            if (i3 > 0 && round > width) {
            ...
            val i4: Int = (i2 - (round / 2)) + ((Int) this.mapOffsetY)
            ...
            if (this.userLocations != null) {
                ...
                val immutableVector: ImmutableVector = this.userLocations.myAvatarPosition
    ...
    override fun onMeasure(i: Int, i2: Int) {
        ...
        val min: Int = Math.min(this.displaySize.x, this.displaySize.y)
     ...
     public override fun onTouchEvent(motionEvent: MotionEvent): Boolean {
        ...
        switch (motionEvent.getActionMasked()) {
            ...
            case 1:
                ...
                break
            case 2:
                val findPointerIndex: Int = motionEvent.findPointerIndex(this.activePointerId)
                val x: Float = motionEvent.getX(findPointerIndex)
                val y: Float = motionEvent.getY(findPointerIndex)
            ...
            case 3:
                ...
                break
            case 6:
                val actionIndex: Int = motionEvent.getActionIndex()
    ...
    /* access modifiers changed from: package-private */
    fun setOnUserClickListener(onUserClickListener2: OnUserClickListener) {
