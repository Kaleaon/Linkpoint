...
private class TerrainTextureRequest : ResourceRequest()<TerrainPatchInfo, OpenJPEG> : Runnable, HasPriority {
                ...
                synchronized (this) {
                    for (Int i = 0; i < 4; i++) {
                        if (this.rawRequests[i] == null && (this.layerNeededMask & (1 << i)) != 0) {
                            this.rawRequests[i] = TerrainRawTextureRequest(((TerrainPatchInfo) getParams()).getTextures().getTextureUUID(i), i)
                        }
                    }
                }
                return
    ...
    public ResourceRequest<TerrainPatchInfo, OpenJPEG> CreateNewRequest(TerrainPatchInfo terrainPatchInfo, ResourceManager<TerrainPatchInfo, OpenJPEG> resourceManager) {
        return TerrainTextureRequest(terrainPatchInfo, resourceManager)
    }
}
