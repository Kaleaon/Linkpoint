...
class FilamentTerrainRenderer(
    ...
    companion object {
        ...
        private const val TERRAIN_SIZE = 256
        private const val PATCHES_PER_EDGE = 16
        private const val PATCH_SIZE = 16
        private const val VERTICES_PER_PATCH = PATCH_SIZE + 1
    ...
    fun initialize() {
        terrainMaterial = materialManager.getMaterial(FilamentMaterialManager.MaterialType.TERRAIN)
    ...
    private fun createTerrainPatch(patchX: Int, patchY: Int, terrainData: TerrainData) {
        try {
            ...
            RenderableManager.Builder(1)
                .boundingBox(Box(minX, minY, MIN_HEIGHT, maxX, maxY, MAX_HEIGHT))
            ...
            val patchIndex = patchY * PATCHES_PER_EDGE + patchX
            terrainPatches[patchIndex] = TerrainPatch(entity, vertexBuffer, indexBuffer, patchX, patchY)
    ...
    private fun createPatchGeometry(
        ...
        val vertexSize = 20
        val vertexData = ByteBuffer.allocateDirect(vertexCount * vertexSize).order(ByteOrder.nativeOrder())
                    ...
                    try {
                        ...
                        if (heightMap != null) {
                            ...
                            if (index < heightMap.heights.size) {
                                height = heightMap.heights[index]
                            }
                        }
                    } catch (e: Exception) {
                        // ignore
        ...
        val indexCount = PATCH_SIZE * PATCH_SIZE * 6
        val indexData = ByteBuffer.allocateDirect(indexCount * 2).order(ByteOrder.nativeOrder())
    ...
    fun updatePatch(patchX: Int, patchY: Int, terrainData: TerrainData) {
        ...
        if (patch != null) {
            ...
            if (instance != 0) {
                rm.setGeometryAt(instance, 0, PrimitiveType.TRIANGLES, newVertexBuffer, newIndexBuffer)
            }
