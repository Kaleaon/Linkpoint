...
public class GridConnectionService extends Service implements OnSharedPreferenceChangeListener {
    ...
    public static final String LOGIN_ACTION = "com.linkpoint.ACTION_LOGIN";
        ...
        if (this.voicePluginServiceConnection == null) {
            ...
            if (!bindService) {
                ...
                if (userManager != null && VoicePluginServiceConnection.shouldDisplayInstallOffer()) {
                    userManager.getChatterList().getActiveChattersManager().HandleChatEvent(ChatterID.getLocalChatterID(userManager.getUserID()), new SLVoiceUpgradeEvent(userManager.getUserID(), LinkpointApp.getContext().getString(R.string.plugin_install_for_voice_needed), true, LicenseChecker.VOICE_PLUGIN_URL), false);
                }
            }
        }
        if (this.voicePluginServiceConnection != null) {
    ...
    private void updateCloudSyncStatus() {
        if (!this.cloudSyncEnabled || this.cloudSyncUserManager == null) {
            ...
            Intent intent = new Intent();
            intent.setComponent(new ComponentName("com.linkpoint.cloud", "com.linkpoint.cloud.DriveSyncService"));
            try {
