...
object MessageIdRegistry {
    ...
    const val AGENT_UPDATE = AgentMessages.AGENT_UPDATE
    
    /** LayerData - Terrain, wind, and cloud data */
    const val LAYER_DATA = 11
    
    /** ObjectUpdate - Full object properties */
    const val OBJECT_UPDATE = ObjectMessages.OBJECT_UPDATE
    
    /** ObjectUpdateCompressed - Compressed object update */
    const val OBJECT_UPDATE_COMPRESSED = ObjectMessages.OBJECT_UPDATE_COMPRESSED
    
    /** ObjectUpdateCached - Server indicates cached object, client must request full data */
    const val OBJECT_UPDATE_CACHED = ObjectMessages.OBJECT_UPDATE_CACHED
    
    /** ImprovedTerseObjectUpdate - Minimal position update */
    const val IMPROVED_TERSE_OBJECT_UPDATE = ObjectMessages.IMPROVED_TERSE_OBJECT_UPDATE
    
    /** KillObject - Object removed from scene */
    const val KILL_OBJECT = ObjectMessages.KILL_OBJECT
    ...
    const val AGENT_THROTTLE = AgentMessages.AGENT_THROTTLE            // Wire: FF FF 00 51 (Lumiya-verified: put(0), put(81))
    ...
    const val COMPLETE_AGENT_MOVEMENT = AgentMessages.COMPLETE_AGENT_MOVEMENT   // Wire: FF FF 00 F9
    
    /** AgentMovementComplete - Sim confirms agent movement */
    const val AGENT_MOVEMENT_COMPLETE = AgentMessages.AGENT_MOVEMENT_COMPLETE   // Wire: FF FF 00 FA
    ...
    const val AGENT_ANIMATION = AgentMessages.AGENT_ANIMATION  // Wire: FF FF 02 1F = -64993 (Linkpoint-specific, unique reserved ID)
    
    /** AgentSetAppearance - Update avatar appearance */
    const val AGENT_SET_APPEARANCE = AgentMessages.AGENT_SET_APPEARANCE  // Wire: FF FF 00 54 = -65452 (Lumiya: AgentSetAppearance)
    
    /** AgentIsNowWearing - Notify what agent is wearing */
    const val AGENT_IS_NOW_WEARING = AgentMessages.AGENT_IS_NOW_WEARING      // Wire: FF FF 01 7F (Lumiya-verified: put(1), put(127))
    
    /** AgentRequestSit - Request to sit on object (HIGH frequency) */
    const val AGENT_REQUEST_SIT = AgentMessages.AGENT_REQUEST_SIT                            // Wire: 06 (Lumiya-verified: put(6), high frequency)
    
    /** AgentSit - Confirm sitting */
    const val AGENT_SIT = AgentMessages.AGENT_SIT  // Wire: 0x07 (high frequency) (Lumiya: AgentSit)
    
    /** AgentDataUpdate - Agent data updated (groups, title, active group) */
    const val AGENT_DATA_UPDATE = AgentMessages.AGENT_DATA_UPDATE         // Wire: FF FF 01 83 = -65149
    ...
    const val TELEPORT_LANDMARK_REQUEST = TeleportMessages.TELEPORT_LANDMARK_REQUEST  // Wire: FF FF 00 41 = -65471 (Lumiya: TeleportLandmarkRequest)
    ...
    const val TELEPORT_LOCATION_REQUEST = TeleportMessages.TELEPORT_LOCATION_REQUEST  // Wire: FF FF 00 3F = -65473 (Lumiya: TeleportLocationRequest)
    ...
    const val AGENT_PAUSE = AgentMessages.AGENT_PAUSE                // Wire: FF FF 00 4E (Lumiya-verified: put(0), put(78))
    
    /** AgentResume - Resume agent updates */
    const val AGENT_RESUME = AgentMessages.AGENT_RESUME               // Wire: FF FF 00 4F (Lumiya-verified: follows AgentPause at 0x4E)
    ...
    const val AGENT_WEARABLES_REQUEST = AgentMessages.AGENT_WEARABLES_REQUEST  // Wire: FF FF 01 7D = -65155 (Lumiya: AgentWearablesRequest)
