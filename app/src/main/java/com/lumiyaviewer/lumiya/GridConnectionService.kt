...
class GridConnectionService : Service implements OnSharedPreferenceChangeListener {
    /* renamed from: -com-lumiyaviewer-lumiya-ui-settings-NotificationTypeSwitchesValues */
    private /* synthetic */ int[] f442-com-lumiyaviewer-lumiya-ui-settings-NotificationTypeSwitchesValues = null
    /* renamed from: -com-lumiyaviewer-lumiya-utils-LEDActionSwitchesValues */
    private /* synthetic */ int[] f443-com-lumiyaviewer-lumiya-utils-LEDActionSwitchesValues = null
    String LOGIN_ACTION = "com.lumiyaviewer.lumiya.ACTION_LOGIN";
    private int REQUEST_CODE_UNREAD_NOTIFY = 2131755072
    private SLGridConnection gridConnection = null
    private String gridName = "Second Life";
    private NotificationSettings notifyGroup = new NotificationSettings(NotificationType.Group)
    private NotificationSettings notifyLocalChat = new NotificationSettings(NotificationType.LocalChat)
    private NotificationSettings notifyPrivate = new NotificationSettings(NotificationType.Private)
    ...
    private Set<Activity> visibleActivities = Collections.synchronizedSet(new HashSet())
    private val cloudPluginInstalledReceiver: BroadcastReceiver = new BroadcastReceiver() {
    ...
    private var cloudPluginReceiverRegistered: Boolean = false
    ...
    private val currentLocationInfo: SubscriptionData<SubscriptionSingleKey, CurrentLocationInfo> = new SubscriptionData(UIThreadExecutor.getInstance(), () -> { /* TODO: fix lambda */ })
    private val eventBus: EventBus = EventBus.getInstance()
    private var licenseCheckHandler: Handler = new Handler() {
    ...
    private val mBinder: IBinder = new GridServiceBinder()
    private val mHandler: Handler = new Handler()
    private val onActiveAgentNameUpdated: OnChatterNameUpdated = new com.lumiyaviewer.lumiya.-$Lambda$3DowF6pLKgVjVrTY9aZKQ2J3cf0.AnonymousClass2(this)
    private var onlineNotificationInfo: OnlineNotificationInfo = new OnlineNotificationInfo(onlineNotify, this, gridName, gridConnection, this.connectedAgentNameRetriever, null)
    private var prefs: SharedPreferences = null
    private val shownNotificationIds: Set<Integer> = Collections.newSetFromMap(Collections.synchronizedMap(new HashMap()))
    private var unreadNotifySubscription: Subscription<Boolean, UnreadNotifications>? = null
    private val voicePluginInstalledReceiver: BroadcastReceiver = new BroadcastReceiver() {
        fun onReceive(context: Context, intent: Intent): Unit {
    ...
    private var voicePluginReceiverRegistered: Boolean = false
    ...
    class GridServiceBinder : Binder {
        ...
        fun getGridConn(): SLGridConnection {
    ...
    private fun connectToVoicePlugin(voiceLoginInfo: VoiceLoginInfo, userManager: UserManager): Unit {
    ...
    private fun hideUnreadNotificationSingle(i: Int): Unit {
    ...
    private fun onCurrentLocationInfo(currentLocationInfo: CurrentLocationInfo): Unit {
        updateOnlineNotification()
    }

    private fun onUnreadNotification(unreadNotifications: UnreadNotifications): Unit {
        showUnreadNotification(unreadNotifications)
    }

    private fun performLogin(sLAuthParams: SLAuthParams): Unit {
        gridName = sLAuthParams.gridName
        gridConnection.Connect(sLAuthParams)
    }

    private fun readPreferences(sharedPreferences: SharedPreferences): Unit {
    ...
    fun setActivityVisible(activity: Activity, z: Boolean): Unit {
        if (z) {
    ...
    private fun showUnreadNotification(unreadNotifications: UnreadNotifications): Unit {
    ...
    private fun showUnreadNotificationSingle(r18: com.lumiyaviewer.lumiya.slproto.users.manager.UnreadNotificationInfo, r19: Int, java.lang.String: @javax.annotation.Nonnull, java.lang.String: @android.support.annotation.Nullable, r22: Boolean, java.lang.String: @android.support.annotation.Nullable): Unit {
        /*
        r17 = this
        r2 = "notification";
        r0 = r17
    ...
    private fun startCloudSync(userManager: UserManager): Unit {
        this.cloudSyncUserManager = userManager
        updateCloudSyncStatus()
    }

    private fun stopCloudSync(): Unit {
        this.cloudSyncUserManager = null
        updateCloudSyncStatus()
    }

    private fun updateCloudSyncStatus(): Unit {
    ...
    private fun updateOnlineNotification(): Unit {
        if (gridConnection != null) {
    ...
    fun enableVoiceMic(z: Boolean): Unit {
        if (this.voicePluginServiceConnection != null) {
    ...
    @EventHandler
    fun handleConnectEvent(sLLoginResultEvent: SLLoginResultEvent): Unit {
    ...
    @EventHandler
    fun onGlobalPreferencesChanged(globalOptionsChangedEvent: GlobalOptionsChangedEvent): Unit {
        readPreferences(globalOptionsChangedEvent.preferences)
    }

    fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences, str: String): Unit {
    ...
    fun startVoice(voiceLoginInfo: VoiceLoginInfo, userManager: UserManager): Unit {
