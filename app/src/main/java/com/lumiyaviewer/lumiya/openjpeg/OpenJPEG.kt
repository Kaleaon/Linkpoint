...
class OpenJPEG : GLTexture {
    ...
    companion object {
        ...
        @JvmStatic external fun applyFlexibleMorph(
            dst: ByteBuffer, src: ByteBuffer, count: Int, weights: FloatArray
        )
        
        @JvmStatic external fun applyMeshMorph(
            weight: Float, positions: ByteBuffer, normals: ByteBuffer, 
            vertexCount: Int, morphPositions: ByteBuffer, morphNormals: ByteBuffer,
            morphTexCoords: ByteBuffer, morphVertexCount: Int, positionOffset: Int,
            normalOffset: Int, output: ByteBuffer
        )
        
        @JvmStatic external fun applyMorphingTransform(
            count: Int, positions: ByteBuffer, normals: ByteBuffer,
            output: ByteBuffer, indices: IntArray, weights: FloatArray
        )
        
        @JvmStatic external fun applyRiggedMeshMorph(
            vertices: ByteBuffer, vertexCount: Int, boneMatrices: FloatArray,
            boneWeights: FloatArray, boneIndices: ByteBuffer, output: ByteBuffer,
            stride: Int
        )
        
        @JvmStatic
        fun bakeTerrain(
            width: Int, height: Int, textures: Array<OpenJPEG?>,
            blendFactors: FloatArray, startX: Int, startY: Int
        ): OpenJPEG {
        ...
        @JvmStatic external fun calcFlexiSections(
            output: FloatArray, sections: Int, xScale: FloatArray, yScale: FloatArray,
            scaleCount: Int, twist: Float, topShearX: Float, topShearY: Float,
            taperX: Float, taperY: Float, radiusOffset: Float, revolutions: Float,
            skew: Float, pathCut: Float, pathCutEnd: Float, hollow: Boolean
        )
        
        @JvmStatic external fun checkFrustrumOcclusion(
            mvpMatrix: FloatArray, bounds: FloatArray, centerX: Float,
            centerY: Float, centerZ: Float
        ): Int
        
        @JvmStatic external fun getFlexiDataSize(sections: Int): Int
        
        @JvmStatic external fun meshPrepareInfluenceBuffer(
            vertices: ByteBuffer, vertexCount: Int, influences: ByteBuffer, stride: Int
        )
        
        @JvmStatic external fun meshPrepareSeparateInfluenceBuffer(
            vertices: ByteBuffer, vertexCount: Int, boneIndices: ByteBuffer,
            boneWeights: ByteBuffer, stride: Int
        )
    }
    ...
    constructor(
        width: Int, height: Int, numComponents: Int, 
        bytesPerPixel: Int, numExtraComponents: Int, flags: Int
    ) {
        this.width = width
        this.height = height
    ...
    @Throws(IOException::class)
    constructor(
        file: File, textureClass: TextureClass, 
        imageFormat: ImageFormat, compressedTextures: Boolean
    ) {
        if (!file.exists()) {
            throw IOException("File does not exist: ${file.absolutePath}")
        }
    ...
    @Throws(IOException::class)
    constructor(
        inputStream: InputStream, imageFormat: ImageFormat,
        flipVertical: Boolean, premultiplyAlpha: Boolean,
        gammaR: Float, gammaG: Float, useMipMaps: Boolean
    ) {
        when (imageFormat) {
            ImageFormat.TGA -> {
            ...
            ImageFormat.KTX2 -> {
                ...
                // Get dimensions
                val dimensions = getKTX2Dimensions(ktx2Data)
    ...
    private external fun decompress(
        path: String, discardLevel: Int, components: Int,
        useMipMaps: Boolean, maxWidth: Int, maxHeight: Int
    ): ByteBuffer?
    
    private external fun decompressTGA(
        data: ByteArray, flipVertical: Boolean, premultiplyAlpha: Boolean,
        gammaR: Float, gammaG: Float, useMipMaps: Boolean
    ): ByteBuffer?
    ...
    private external fun writeJPEG2K(
        path: String, buffer: ByteBuffer, width: Int, height: Int,
        numComponents: Int, numExtraComponents: Int
    ): Int
    
    private external fun writeRaw(buffer: ByteBuffer, path: String)
    
    private external fun drawBuf(
        dstBuffer: ByteBuffer, dstWidth: Int, dstHeight: Int, dstComponents: Int,
        srcBuffer: ByteBuffer, srcWidth: Int, srcHeight: Int, srcComponents: Int,
        offset: Int, tile: Boolean, blendAlpha: Boolean, 
        flipVertical: Boolean, isBump: Boolean
    )
    
    private external fun setComponentBuf(
        buffer: ByteBuffer, width: Int, height: Int, 
        numComponents: Int, numExtraComponents: Int, component: Int, value: Byte
    )
    
    private external fun bakeTerrainRaw(
        output: ByteBuffer, width: Int, height: Int,
        textures: Array<ByteBuffer?>, widths: IntArray, heights: IntArray,
        components: IntArray, blendFactors: FloatArray, startX: Int, startY: Int
    )
    
    // KTX2/Basis Universal transcoding methods
    private external fun initBasisTranscoder(): Boolean
    private external fun decompressKTX2(ktx2Data: ByteArray, targetFormat: Int): ByteBuffer?
    ...
    fun compressETC1(): Boolean {
        ...
        val compressed = allocateRaw(encodedSize)
            ?: throw IOException("Out of memory for $encodedSize allocation")
        
        ETC1.encodeImage(
            rawBuffer, width, height, bytesPerPixel, 
            width * bytesPerPixel, compressed
        )
    ...
    fun setAsImmutableTexture(): Int {
        rawBuffer?.let { buffer ->
            if (bytesPerPixel == ETC1_BYTES_PER_PIXEL) {
                ...
                val (internalFormat, format, type) = when (numComponents) {
                    1 -> Triple(GLES30.GL_R8, GLES30.GL_RED, GLES30.GL_UNSIGNED_BYTE)
                    3 -> if (bytesPerPixel == 2) {
        ...
        rawBuffer?.let { buffer ->
            if (bytesPerPixel == ETC1_BYTES_PER_PIXEL) {
                GLES10.glCompressedTexImage2D(
                    GLES10.GL_TEXTURE_2D, 0, ETC1.ETC1_RGB8_OES,
                    width, height, 0, buffer.capacity(), buffer
                )
            } else {
                val format = when (numComponents) {
                ...
                val type = if (bytesPerPixel == 2 && numComponents == 3) {
        ...
        rawBuffer?.let { buffer ->
            if (bytesPerPixel == ETC1_BYTES_PER_PIXEL) {
                ...
                GLES20.glCompressedTexImage2D(
                    target, 0, ETC1.ETC1_RGB8_OES,
                    width, height, 0, capacity, buffer
                )
                return capacity
            } else {
                val format = when (numComponents) {
     ...
     */
    fun blendAlpha(source: OpenJPEG, flipVertical: Boolean) {
     ...
     */
    fun draw(source: OpenJPEG, offset: Int, tile: Boolean) {
        if (rawBuffer != null && source.rawBuffer != null) {
            drawBuf(
                rawBuffer!!, width, height, numComponents,
                source.rawBuffer!!, source.width, source.height, source.numComponents,
     ...
     */
    fun drawBump(source: OpenJPEG, offset: Int, tile: Boolean, flipVertical: Boolean) {
        if (rawBuffer != null && source.rawBuffer != null &&
    ...
    fun getExtraAsBitmap(): Bitmap {
        ...
        for (y in 0 until height) {
            for (x in 0 until width) {
                val pixel = if (numExtraComponents == 1) {
     ...
     * Puts a pixel row
     */
    fun putPixelRow(row: Int, pixels: IntArray, count: Int) {
        rawBuffer?.let { buffer ->
    ...
    fun setComponent(component: Int, value: Byte) {
        rawBuffer?.let { buffer ->
            setComponentBuf(
                buffer, width, height, numComponents, 
                numExtraComponents, component, value
            )
        }
    }
    ...
    override fun hasAlphaLayer(): Boolean = 
        bytesPerPixel != ETC1_BYTES_PER_PIXEL && (numComponents >= 4 || numComponents == 1)
