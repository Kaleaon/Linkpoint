...
class SLNotecard {
    ...
    private class AttachmentClickableSpan extends ClickableSpan implements InventoryEntrySpan {
        private OnAttachmentClickListener clickListener
        private SLInventoryEntry entry

        AttachmentClickableSpan(SLInventoryEntry sLInventoryEntry, OnAttachmentClickListener onAttachmentClickListener) {
    ...
    private class AttachmentSpan extends ReplacementSpan implements InventoryEntrySpan {
        private SLInventoryEntry entry
        private String linkText

        AttachmentSpan(SLInventoryEntry sLInventoryEntry) {
            this.entry = sLInventoryEntry
            this.linkText = sLInventoryEntry.getReadableTextForLink()
        }

        void draw(Canvas canvas, CharSequence charSequence, int i, int i2, float f, int i3, int i4, int i5, Paint paint) {
            if (i != i2) {
                Paint paint2 = fun Paint(): new
        ...
        int getSize(Paint paint, CharSequence charSequence, int i, int i2, Paint.FontMetricsInt fontMetricsInt) {
    ...
    private class NotecardAttachment {
        ...
        NotecardAttachment(int i, SLInventoryEntry sLInventoryEntry) {
            this.extCharIndex = i
            this.entry = sLInventoryEntry
        }
    }
    ...
    SLNotecard(Spanned spanned, boolean z) {
        this.isScript = z
        StringBuilder sb = fun StringBuilder(): new
        this.attachments = fun ArrayList(): new
    ...
    SLNotecard(boolean z) {
        this.isScript = z
        this.attachments = fun ArrayList(): new
        this.notecardText = "";
    }

    SLNotecard(byte[] bArr, boolean z) throws SimpleStringParser.StringParsingException {
        String stringFromVariableUTF = SLMessage.stringFromVariableUTF(bArr)
        this.isScript = z
        if (!z) {
            SimpleStringParser simpleStringParser = fun SimpleStringParser(): new
    ...
    android.text.SpannableStringBuilder toSpannableString(boolean r7, com.lumiyaviewer.lumiya.slproto.assets.SLNotecard.OnAttachmentClickListener r8) {
        /*
            r6 = this
            r0 = 0
