...
class SLAvatarAppearance : SLModule : SLWearable.OnWearableStatusChangeListener {
    private Int Param_agentSizeVPHeadSize = 682
    private Int Param_agentSizeVPHeelHeight = 198
    private Int Param_agentSizeVPHeight = 33
    private Int Param_agentSizeVPHipLength = 842
    private Int Param_agentSizeVPLegLength = 692
    private Int Param_agentSizeVPNeckLength = 756
    private Int Param_agentSizeVPPlatformHeight = 503
    ...
    private SLCaps caps
    private AtomicReference<UUID> cofFolderUUID = AtomicReference<>()
    ...
    private SubscriptionData<InventoryQuery, InventoryEntryList> currentOutfitFolder
    private SubscriptionData<InventoryQuery, InventoryEntryList> findCofFolder
    private SLInventory inventory
    ...
    private AtomicBoolean needUpdateCOF = AtomicBoolean(false)
    ...
    private AtomicReference<Map<UUID, String>> wantedAttachments = AtomicReference<>(ImmutableMap.of())
    ...
    private RequestHandler<SubscriptionSingleKey> wornItemsRequestHandler = AsyncRequestHandler(this.agentCircuit, SimpleRequestHandler<SubscriptionSingleKey>() {
    ...
    ResultHandler<SubscriptionSingleKey, ImmutableList<WornItem>> wornItemsResultHandler
    @Nonnull
    ...
    class WornItem {
        ...
        Int getObjectLocalID() {
            return this.objectLocalID
        }

        SLWearableType getWornOn() {
            return this.wornOn
        }
    ...
    SLAvatarAppearance(SLAgentCircuit sLAgentCircuit, SLInventory sLInventory, SLCaps sLCaps) {
        ...
        this.findCofFolder = SubscriptionData<>(sLAgentCircuit, Subscription.OnData(this) {
            ...
            /* renamed from: -$f0 */
            private /* synthetic */ Any f118$f0

            private /* synthetic */ Unit $m$0(
/*
Method generation error in method: com.lumiyaviewer.lumiya.slproto.modules.-$Lambda$Jp5Too8LbDpaKzeYKjkvQvC1hZo.1.$m$0(java.lang.Any):Unit, dex: classes.dex
            jadx.core.utils.exceptions.JadxRuntimeException: Method args not loaded: com.lumiyaviewer.lumiya.slproto.modules.-$Lambda$Jp5Too8LbDpaKzeYKjkvQvC1hZo.1.$m$0(java.lang.Any):Unit, class status: UNLOADED
...
*/
    ...
    private Unit UpdateCOFContents() {
        ...
        if ((areWearablesReady ? this.cofReady : false) && (data = this.currentOutfitFolder.getData()) != null && (folder = data.getFolder()) != null && this.needUpdateCOF.getAndSet(false)) {
            ...
            Debug.Printf("Update COF: addWearablesList %d, killList %d", Int.valueOf(hashMap.size()), Int.valueOf(linkedList.size()))
    ...
    private Unit UpdateCurrentOutfitLink(@Nonnull InventoryEntryList inventoryEntryList) {
    ...
    private synchronized Unit UpdateMultiLayer() {
        ...
        Debug.Printf("AvatarAppearance: MultiLayer: Updating multi layer appearance.", Any[0])
        ...
        if (!(data == null || database == null)) {
            ...
            if (WearItemList(database, linkedList, false)) {
                Debug.Printf("AvatarAppearance: MultiLayer: had some extra layers.", Any[0])
                SendAgentIsNowWearing()
                StartUpdatingAppearance()
            } else {
                Debug.Printf("AvatarAppearance: MultiLayer: no extra layers.", Any[0])
            }
            if (linkedList2.size() != 0) {
                Debug.Printf("AvatarAppearance: Re-attaching %d attachments from COF.", Int.valueOf(linkedList2.size()))
                HashMap hashMap = HashMap()
                ...
                Debug.Printf("AvatarAppearance: No attachments in COF.", Any[0])
            }
        }
    ...
    private synchronized Unit UpdateServerSideAppearance(String str, Int i) {
        Debug.Printf("AvatarAppearance: capURL '%s', cofVersion %d", str, Int.valueOf(i))
        ...
        this.serverSideAppearanceUpdateTask = GenericHTTPExecutor.getInstance().submit(Runnable(i, this, str) {
            ...
            /* renamed from: -$f0 */
            private /* synthetic */ Int f119$f0

            /* renamed from: -$f1 */
            private /* synthetic */ Any f120$f1

            /* renamed from: -$f2 */
            private /* synthetic */ Any f121$f2
...
*/
    ...
    private Boolean areWearablesReady() {
        ...
        while (i < length) {
            ...
            if (!isCritical) {
                ...
                objArr[1] = Int.valueOf(row != null ? row.size() : 0)
        ...
        for (SLWearable wearableData : this.wornWearables.values()) {
            ...
            if (wearableData2 != null) {
                for (SLWearableData.WearableParam wearableParam : wearableData2.params) {
                    SLAvatarParams.ParamSet paramSet2 = SLAvatarParams.paramByIDs.get(Int.valueOf(wearableParam.paramIndex))
    ...
    /* access modifiers changed from: private */
    ImmutableList<WornItem> getWornItems() {
        SLObjectAvatarInfo agentAvatar
        ImmutableList.Builder builder = ImmutableList.builder()
        for (Table.Cell cell : this.wornWearables.cellSet()) {
            SLWearable sLWearable = (SLWearable) cell.getValue()
            if (sLWearable != null) {
                builder.add((Any) WornItem((SLWearableType) cell.getRowKey(), 0, (UUID) cell.getColumnKey(), sLWearable.getName(), 0, false))
            }
        }
        if (!(this.parcelInfo == null || (agentAvatar = this.parcelInfo.getAgentAvatar()) == null)) {
            try {
                for (SLObjectInfo next : agentAvatar.treeNode) {
                    builder.add((Any) WornItem((SLWearableType) null, next.attachmentID, next.getId(), next.getName(), next.localID, next.isTouchable()))
                }
            } catch (NoSuchElementException e) {
                Debug.Warning(e)
            }
    ...
    Unit m201com_lumiyaviewer_lumiya_slproto_modules_SLAvatarAppearancemthref1(InventoryEntryList inventoryEntryList) {
    ...
    Unit m200com_lumiyaviewer_lumiya_slproto_modules_SLAvatarAppearancemthref0(InventoryEntryList inventoryEntryList) {
        SLInventoryEntry folder
    ...
    Unit AttachInventoryItem(SLInventoryEntry sLInventoryEntry, Int i, Boolean z) {
        InventoryDB inventoryDB = null
        if (this.userManager != null) {
            inventoryDB = this.userManager.getInventoryManager().getDatabase()
        }
    ...
    Unit ChangeOutfit(List<SLInventoryEntry> list, Boolean z, SLInventoryEntry sLInventoryEntry) {
        RezMultipleAttachmentsFromInv rezMultipleAttachmentsFromInv
        SLWearableType byCode
        SLWearable remove
        RezMultipleAttachmentsFromInv rezMultipleAttachmentsFromInv2
        ...
        Debug.Printf("Wearing: totalAttachments %d", Int.valueOf(arrayList.size()))
    ...
    Unit DetachInventoryItem(SLInventoryEntry sLInventoryEntry) {
    ...
    Unit DetachItem(WornItem wornItem) {
        if (canDetachItem(wornItem)) {
            DetachItem(wornItem.objectLocalID)
        }
    }

    Unit DetachItemFromPoint(Int i) {
        SLObjectAvatarInfo agentAvatar
            ...
            try {
                for (SLObjectInfo next : agentAvatar.treeNode) {
                    if (next.attachedToUUID != null && (!next.isDead) && next.attachmentID == i && this.agentCircuit.getModules().rlvController.canDetachItem(i, next.getId())) {
                        ...
                        hashSet.add(Int.valueOf(next.localID))
                    }
                    hashSet = hashSet
                }
            } catch (NoSuchElementException e) {
        ...
        if (hashSet != null) {
            for (Int intValue : hashSet) {
                DetachItem(intValue.intValue())
            }
        }
    }

    Unit ForceTakeItemOff(SLWearableType sLWearableType) {
    ...
    Unit HandleAvatarAppearance(AvatarAppearance avatarAppearance) {
        if (avatarAppearance.AppearanceData_Fields.size() > 0) {
            ...
            Debug.Printf("AvatarAppearance: inventory COF %d, last updated COF %d, appearance COF %d", Int.valueOf(this.currentCofInventoryVersion), Int.valueOf(this.lastCofUpdatedVersion), Int.valueOf(this.currentCofAppearanceVersion))
        }
    }
    ...
    Unit HandleCircuitReady() {
        ...
        if (this.userManager != null) {
            ...
            if (!z) {
                Debug.Printf("Existing COF folder not found, requesting.", Any[0])
    ...
    Unit OnMyAvatarCreated(SLObjectAvatarInfo sLObjectAvatarInfo) {
        if (this.agentVisualParams != null) {
            sLObjectAvatarInfo.ApplyAvatarVisualParams(this.agentVisualParams)
        }
    }

    Unit SendAgentWearablesRequest() {
    ...
    Unit TakeItemOff(SLInventoryEntry sLInventoryEntry) {
        InventoryDB inventoryDB = null
        if (this.userManager != null) {
            inventoryDB = this.userManager.getInventoryManager().getDatabase()
        }
    ...
    Unit WearItem(SLInventoryEntry sLInventoryEntry, Boolean z) {
        InventoryDB inventoryDB = null
        if (this.userManager != null) {
            inventoryDB = this.userManager.getInventoryManager().getDatabase()
        }
    ...
    Boolean canDetachItem(SLInventoryEntry sLInventoryEntry) {
        if (sLInventoryEntry.assetType == SLAssetType.AT_LINK.getTypeCode()) {
            if (sLInventoryEntry.invType == SLInventoryType.IT_WEARABLE.getTypeCode()) {
                return true
            }
    ...
    Boolean canDetachItem(WornItem wornItem) {
    ...
    Boolean canTakeItemOff(SLInventoryEntry sLInventoryEntry) {
    ...
    Boolean canWearItem(SLInventoryEntry sLInventoryEntry) {
        InventoryDB inventoryDB = null
        if (this.userManager != null) {
            inventoryDB = this.userManager.getInventoryManager().getDatabase()
        }
    ...
    Unit finishBaking(BakeProcess bakeProcess2, SLTextureEntry sLTextureEntry) {
    ...
    Boolean hasWornWearable(SLWearableType sLWearableType) {
        return this.wornWearables.containsRow(sLWearableType)
    }

    Boolean isItemWorn(SLInventoryEntry sLInventoryEntry) {
    ...
    /* synthetic */ Unit m202lambda$com_lumiyaviewer_lumiya_slproto_modules_SLAvatarAppearance_17963(Int i, String str) {
            ...
            try {
                ...
                if (PerformRequest != null && PerformRequest.keyExists("error")) {
                    ...
                    if (byKey.isString()) {
                        ...
                        Debug.Printf("AvatarAppearance: server-side update ok.", Any[0])
                    }
                }
                this.lastCofUpdateError = false
                return
