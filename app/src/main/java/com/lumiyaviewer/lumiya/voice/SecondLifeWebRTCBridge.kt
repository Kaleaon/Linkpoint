...
class SecondLifeWebRTCBridge {
    private String TAG = "SLWebRTCBridge"
    
    private Context context
    private OkHttpClient httpClient
    private Gson gson
    private WebRTCVoiceAdapter voiceAdapter
    
    // Second Life voice server endpoints
    private String SL_VOICE_PROVISION_URL = "https://cap.secondlife.com/cap/"
    private String SL_VOICE_ACCOUNT_URL = "https://bhr.vivox.com/api2/viv_signin.php"
    ...
    SecondLifeWebRTCBridge(Context context) {
    ...
    private CompletableFuture<Boolean> authenticateWithVoiceServer() {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                JsonObject authRequest = JsonObject()
     ...
     */
    CompletableFuture<Boolean> disconnectFromVoice() {
        return CompletableFuture.supplyAsync(() -> {
            try {
     ...
     */
    CompletableFuture<Boolean> processVoiceServerCaps(String capsUrl, String sessionId) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                Log.i(TAG, "Processing voice server CAPS: " + capsUrl)
                
                // Create CAPS request for voice provisioning
                JsonObject capsRequest = JsonObject()
