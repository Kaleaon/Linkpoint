...
class Huffman
{
    ...
    byte[] decode(final byte[] array) {
        ...
        while (j > 0) {
            final Node node2 = node.children[n << 8 - j & 0xFF];
    ...
    void encode(final ByteString byteString, final BufferedSink bufferedSink) throws IOException {
        ...
        while (i < byteString.size()) {
            ...
            while (j >= 8) {
                j -= 8;
                bufferedSink.writeByte((int)(n >> j));
            }
            ++i;
        }
        if (j > 0) {
            bufferedSink.writeByte((int)((long)(255 >>> j) | n << 8 - j));
        }
    }
