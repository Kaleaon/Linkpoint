...
final class BackStackRecord extends FragmentTransaction implements BackStackEntry, OpGenerator
{
        ...
        if (!class1.isAnonymousClass() && Modifier.isPublic(modifiers) && (!class1.isMemberClass() || Modifier.isStatic(modifiers))) {
            ...
            if (str != null) {
                if (obj.mTag != null && !str.equals(obj.mTag)) {
                    throw new IllegalStateException("Can't change tag of fragment " + obj + ": was " + obj.mTag + " now " + str);
                }
                obj.mTag = str;
            }
            if (mContainerId != 0) {
                if (mContainerId == -1) {
                    throw new IllegalArgumentException("Can't add fragment " + obj + " with tag " + str + " to container view with no id");
                }
                if (obj.mFragmentId != 0 && obj.mFragmentId != mContainerId) {
                    throw new IllegalStateException("Can't change container ID of fragment " + obj + ": was " + obj.mFragmentId + " now " + mContainerId);
                }
            ...
            for (int i = 0; i < size; ++i) {
                ...
                Label_0189:
                    while (true) {
                        ...
                        final BackStackRecord backStackRecord = list.get(index);
    ...
    public void runOnCommitRunnables() {
        if (this.mCommitRunnables != null) {
            for (int size = this.mCommitRunnables.size(), i = 0; i < size; ++i) {
