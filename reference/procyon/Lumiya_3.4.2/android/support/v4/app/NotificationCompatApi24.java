...
class NotificationCompatApi24
{
    ...
    private static NotificationCompatBase.Action getActionCompatFromAction(final Notification$Action notification$Action, final NotificationCompatBase.Action.Factory factory, final RemoteInputCompatBase.RemoteInput.Factory factory2) {
        boolean b = false;
        final RemoteInputCompatBase.RemoteInput[] compat = RemoteInputCompatApi20.toCompat(notification$Action.getRemoteInputs(), factory2);
        if (notification$Action.getExtras().getBoolean("android.support.allowGeneratedReplies") || notification$Action.getAllowGeneratedReplies()) {
            b = true;
        }
        return factory.build(notification$Action.icon, notification$Action.title, notification$Action.actionIntent, notification$Action.getExtras(), compat, null, b);
    }
