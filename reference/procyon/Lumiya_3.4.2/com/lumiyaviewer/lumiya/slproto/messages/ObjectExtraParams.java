...
public class ObjectExtraParams extends SLMessage
{
    ...
    public int CalcPayloadSize() {
        ...
        while (iterator.hasNext()) {
            n += iterator.next().ParamData.length + 12;
        }
        return n;
    }
    ...
    public void PackPayload(final ByteBuffer byteBuffer) {
        ...
        for (final ObjectData objectData : this.ObjectData_Fields) {
            ...
            this.packShort(byteBuffer, (short)objectData.ParamType);
            this.packBoolean(byteBuffer, objectData.ParamInUse);
            this.packInt(byteBuffer, objectData.ParamSize);
            this.packVariable(byteBuffer, objectData.ParamData, 1);
        }
    }
    ...
    public void UnpackPayload(final ByteBuffer byteBuffer) {
        ...
        for (int i = 0; i < (value & 0xFF); ++i) {
            ...
            e.ParamType = (this.unpackShort(byteBuffer) & 0xFFFF);
            e.ParamInUse = this.unpackBoolean(byteBuffer);
            e.ParamSize = this.unpackInt(byteBuffer);
            e.ParamData = this.unpackVariable(byteBuffer, 1);
