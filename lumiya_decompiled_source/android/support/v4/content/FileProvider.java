            ...
            try {
                ...
                while (true) {
                    ...
                    if (!canonicalPath.startsWith(path)) {
                        entry2 = entry;
                    } else if (entry != null && path.length() <= entry.getValue().getPath().length()) {
                        entry2 = entry;
                    }
                }
                if (entry != null) {
                    String path2 = entry.getValue().getPath();
                    return new Uri.Builder().scheme("content").authority(this.mAuthority).encodedPath(Uri.encode(entry.getKey()) + '/' + Uri.encode(!path2.endsWith("/") ? canonicalPath.substring(path2.length() + 1) : canonicalPath.substring(path2.length()), "/")).build();
                }
