...
public class ViewDragHelper {
    ...
    private boolean checkTouchSlop(View view, float f, float f2) {
        if (view != null) {
            ...
            return (z && z2) ? (f * f) + (f2 * f2) > ((float) (this.mTouchSlop * this.mTouchSlop)) : !z ? z2 && Math.abs(f2) > ((float) this.mTouchSlop) : Math.abs(f) > ((float) this.mTouchSlop);
        }
        return false;
    }
    ...
    private int computeAxisDuration(int i, int i2, int i3) {
        if (i != 0) {
            ...
            int abs = Math.abs(i2);
            return Math.min(abs <= 0 ? (int) (((Math.abs(i) / i3) + 1.0f) * 256.0f) : Math.round(Math.abs(distanceInfluenceForSnapDuration / abs) * 1000.0f) * 4, (int) MAX_SETTLE_DURATION);
        }
        return 0;
    }
    ...
    private int computeSettleDuration(View view, int i, int i2, int i3, int i4) {
        ...
        int i5 = abs3 + abs4;
        int i6 = abs + abs2;
        return (int) (((clampMag == 0 ? abs / i6 : abs3 / i5) * computeAxisDuration(i, clampMag, this.mCallback.getViewHorizontalDragRange(view))) + ((clampMag2 == 0 ? abs2 / i6 : abs4 / i5) * computeAxisDuration(i2, clampMag2, this.mCallback.getViewVerticalDragRange(view))));
    }
