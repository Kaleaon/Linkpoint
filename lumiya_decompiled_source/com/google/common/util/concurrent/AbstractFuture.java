...
public abstract class AbstractFuture<V> implements ListenableFuture<V> {
    ...
    private static final class UnsafeAtomicHelper extends AtomicHelper {
        ...
        static {
            ...
            try {
                ...
                VALUE_OFFSET = unsafe.objectFieldOffset(AbstractFuture.class.getDeclaredField(VrSettingsProviderContract.SETTING_VALUE_KEY));
    ...
    static {
        ...
        try {
            ...
            try {
                synchronizedHelper = new SafeAtomicHelper(AtomicReferenceFieldUpdater.newUpdater(Waiter.class, Thread.class, "thread"), AtomicReferenceFieldUpdater.newUpdater(Waiter.class, Waiter.class, "next"), AtomicReferenceFieldUpdater.newUpdater(AbstractFuture.class, Waiter.class, "waiters"), AtomicReferenceFieldUpdater.newUpdater(AbstractFuture.class, Listener.class, "listeners"), AtomicReferenceFieldUpdater.newUpdater(AbstractFuture.class, Object.class, VrSettingsProviderContract.SETTING_VALUE_KEY));
            } catch (Throwable th2) {
    ...
    public boolean completeWithFuture(ListenableFuture<? extends V> listenableFuture, Object obj) {
        ...
        if (listenableFuture instanceof TrustedFuture) {
            failure = ((AbstractFuture) listenableFuture).value;
        } else {
            try {
                failure = Uninterruptibles.getUninterruptibly(listenableFuture);
                if (failure == null) {
                    failure = NULL;
                }
            } catch (CancellationException e) {
                failure2 = new Cancellation(false, e);
                failure = failure2;
            } catch (ExecutionException e2) {
                failure2 = new Failure(e2.getCause());
                failure = failure2;
            } catch (Throwable th) {
                failure = new Failure(th);
            }
        }
        if (ATOMIC_HELPER.casValue(this, obj, failure)) {
            complete();
            return true;
        }
        return false;
