...
class AvatarAnimationList {
    ...
    public AvatarAnimationList(Collection<AvatarAnimationState> collection) {
        ArrayList arrayList = new ArrayList(collection.size());
        ImmutableList.Builder<AvatarRunningSequence> builder = ImmutableList.builder();
        for (AvatarAnimationState avatarAnimationState : collection) {
            avatarAnimationState.getRunningAnimations(builder, arrayList);
        }
    ...
    public void animate(AvatarSkeleton avatarSkeleton, float[] fArr, float[] fArr2, LLQuaternion[] lLQuaternionArr, LLVector3[] lLVector3Arr) {
        for (AvatarRunningAnimation avatarRunningAnimation : this.animations) {
            avatarRunningAnimation.animate(avatarSkeleton, fArr, fArr2, lLQuaternionArr, lLVector3Arr);
        }
