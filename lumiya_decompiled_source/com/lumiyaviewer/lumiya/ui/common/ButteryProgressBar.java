...
public class ButteryProgressBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.mAnimator.isStarted()) {
            ...
            int width2 = getWidth() >> (this.mSegmentCount - 1);
            int i = 0;
            while (i < this.mSegmentCount) {
                float f = floatValue * (width >> (i + 1));
                canvas.drawRect((f + this.mSolidBarDetentWidth) - width2, 0.0f, (i == 0 ? width + width2 : 2.0f * f) - width2, this.mSolidBarHeight, this.mPaint);
                i++;
            }
        }
    }
