...
public class CameraParams {
    ...
    private void processFling() {
        if (this.isFlinging) {
            ...
            this.tilt = Math.max(Math.min(this.tilt + (this.tiltFlingSpeed * f), (float) MAX_PITCH), (float) MIN_PITCH);
    ...
    public void rotate(float f, float f2) {
        synchronized (this.lock) {
            ...
            this.tilt = Math.max(Math.min(this.tilt + f2, (float) MAX_PITCH), (float) MIN_PITCH);
            this.isFlinging = false;
        }
    }
