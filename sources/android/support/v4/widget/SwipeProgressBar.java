...
final class SwipeProgressBar {
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.mRunning) {
            ...
            float interpolation = INTERPOLATOR.getInterpolation((((float) ((currentAnimationTimeMillis - this.mFinishTime) % 1000)) / 10.0f) / 100.0f) * ((float) (width / 2));
        ...
        if (f >= 0.0f && f <= 25.0f) {
            drawCircle(canvas, (float) i2, (float) i3, this.mColor1, ((25.0f + f) * 2.0f) / 100.0f);
        }
        if (f >= 0.0f && f <= 50.0f) {
            drawCircle(canvas, (float) i2, (float) i3, this.mColor2, (2.0f * f) / 100.0f);
        }
        if (f >= 25.0f && f <= 75.0f) {
