...
public final class BidiFormatter {
    ...
    private static class DirectionalityEstimator {
        ...
        private static byte getCachedDirectionality(char c) {
            return c >= DIR_TYPE_CACHE_SIZE ? Character.getDirectionality(c) : DIR_TYPE_CACHE[c];
        }
        ...
        private byte skipEntityBackward() {
            ...
            while (this.charIndex > 0) {
                ...
                this.charIndex = i2;
                this.lastChar = (char) charSequence.charAt(i2);
                if (this.lastChar != '&') {
                    if (this.lastChar == ';') {
                        break;
                    }
        ...
        public byte dirTypeBackward() {
            ...
            if (!Character.isLowSurrogate(this.lastChar)) {
                ...
                return !this.isHtml ? cachedDirectionality : this.lastChar != '>' ? this.lastChar == ';' ? skipEntityBackward() : cachedDirectionality : skipTagBackward();
            }
        ...
        public byte dirTypeForward() {
            ...
            if (!Character.isHighSurrogate(this.lastChar)) {
                this.charIndex++;
                byte cachedDirectionality = getCachedDirectionality(this.lastChar);
                return !this.isHtml ? cachedDirectionality : this.lastChar != '<' ? this.lastChar == '&' ? skipEntityForward() : cachedDirectionality : skipTagForward();
            }
    ...
    private String markAfter(CharSequence charSequence, TextDirectionHeuristicCompat textDirectionHeuristicCompat) {
        ...
        return (!this.mIsRtlContext && (isRtl || getExitDir(charSequence) == 1)) ? LRM_STRING : !this.mIsRtlContext ? "" : (isRtl && getExitDir(charSequence) != -1) ? "" : RLM_STRING;
    }

    private String markBefore(CharSequence charSequence, TextDirectionHeuristicCompat textDirectionHeuristicCompat) {
        boolean isRtl = textDirectionHeuristicCompat.isRtl(charSequence, 0, charSequence.length());
        return (!this.mIsRtlContext && (isRtl || getEntryDir(charSequence) == 1)) ? LRM_STRING : !this.mIsRtlContext ? "" : (isRtl && getEntryDir(charSequence) != -1) ? "" : RLM_STRING;
    }
    ...
    public CharSequence unicodeWrap(CharSequence charSequence, TextDirectionHeuristicCompat textDirectionHeuristicCompat, boolean z) {
        ...
        if (getStereoReset() && z) {
            spannableStringBuilder.append(markBefore(charSequence, !isRtl ? TextDirectionHeuristicsCompat.LTR : TextDirectionHeuristicsCompat.RTL));
        }
        if (isRtl == this.mIsRtlContext) {
            spannableStringBuilder.append(charSequence);
        } else {
            spannableStringBuilder.append(!isRtl ? LRE : RLE);
            spannableStringBuilder.append(charSequence);
            spannableStringBuilder.append(PDF);
        }
        if (z) {
            spannableStringBuilder.append(markAfter(charSequence, !isRtl ? TextDirectionHeuristicsCompat.LTR : TextDirectionHeuristicsCompat.RTL));
        }
