...
class InventoryManager extends Utils.EventEmitter {
  ...
  async load() {
    ...
    try {
      ...
      this.protocol.sendMessage('FetchInventory', {
  ...
  /**
   * Create demo inventory structure
   */
  createDemoInventory() {
    // Create root folder
    this.rootFolder = {
    ...
    const defaultFolders = [
      ...
      { name: 'Gestures', icon: '👋' },
      { name: 'Landmarks', icon: '📍' },
      { name: 'Lost and Found', icon: '🔍' },
      { name: 'Notecards', icon: '📝' },
      { name: 'Objects', icon: '📦' },
      { name: 'Photos', icon: '📷' },
      { name: 'Scripts', icon: '📜' },
      { name: 'Sounds', icon: '🔊' },
      { name: 'Textures', icon: '🖼️' },
      { name: 'Trash', icon: '🗑️' }
    ];

    defaultFolders.forEach(folder => {
      const folderId = Utils.generateUUID();
      this.folders.set(folderId, {
        id: folderId,
        name: folder.name,
        icon: folder.icon,
        type: 'folder',
        parent: this.rootFolder.id,
        children: []
      });
      this.rootFolder.children.push(folderId);
    });
